...
Press **Option+Shift+Space** anywhere to start recording. Press again to stop — your speech is transcribed and pasted at your cursor (or copied to clipboard). You can customize the shortcut and enable push-to-talk in Settings.
...
```
[Global Hotkey] → Start Recording → [Speak] → [Hotkey / Release] → Stop
...
- Start/stop sounds (toggle in Settings)
- Live recording timer and audio-reactive waveform on the floating pill
...
- App icon in bundle; `./run.sh build` also creates `build/VoiceDictation-0.3.0.zip`
...
```bash
./run.sh build          # Build only (also writes build/VoiceDictation-0.3.0.zip)
