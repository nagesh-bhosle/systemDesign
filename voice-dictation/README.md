...
Known-good **headset + laptop** dictation restore: `git checkout voice-dictation-working` (does not move with 0.4.0).
...
- App icon in bundle; `./run.sh build` also creates `build/VoiceDictation-0.4.0.zip`
...
```bash
./run.sh build          # Build only (also writes build/VoiceDictation-0.4.0.zip)
