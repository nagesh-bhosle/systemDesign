#!/bin/bash
# GoPuff demo — start Docker (Postgres + Redis), build, run Spring Boot.
...
for i in $(seq 1 30); do
    if docker exec gopuff-postgres pg_isready -U gopuff -d gopuff > /dev/null 2>&1; then
...
for i in $(seq 1 20); do
    if docker exec gopuff-redis redis-cli ping 2>/dev/null | grep -q PONG; then
        echo "  Redis is ready"
        break
    fi
    if [ "$i" -eq 20 ]; then
...
echo ""
echo "GoPuff demo is running"
echo "  Open:      http://localhost:8082"
echo "  PostgreSQL: localhost:5434/gopuff"
echo "  Redis:     localhost:6380"
echo "  Strategies: see src/main/resources/application.yml (gopuff.*)"
echo "  Stop:      ./stop.sh"
echo ""
